...
from components.item_result import show_single_item_result
from components.menu_result import show_menu_result

# --- INITIALIZATION ---
if "page" not in st.session_state:
    st.session_state.page = "home"
if "mode" not in st.session_state:
    st.session_state.mode = "Single Item"
...
def run_analysis_logic(image_file, key_prefix):
    ...
    if st.session_state.mode == "Menu Scan":
        ...
        def on_add_menu(s, f):
            st.session_state['sugar_intake'] = st.session_state.get('sugar_intake', 0) + s
    ...
    # 2. SINGLE ITEM MODE
        ...
        if result_data:
            # --- NEW LOGIC: Check if it is a beverage ---
            if result_data.get('is_beverage') is False:
                ...
                # Option to try again immediately
                ...
                show_single_item_result(
                    result_data, 
                    None, # Pass None so the image isn't shown again in the result card
